...
const NexusHeader = memo(({ safeTop, isConn, addr, cpu, ram }: {
  ...
  return (
    <View style={[NH.root, { paddingTop:safeTop }]}>
      ...
      <View style={NH.body}>
        <View style={{ flex:1, gap:3 }}>
          <Text style={NH.eye}>NEXUS COMMAND · PC COCKPIT</Text>
          <View style={{ flexDirection:'row', alignItems:'center', gap:8 }}>
            <MaterialCommunityIcons name="view-dashboard-variant" size={16} color={CYAN} />
            <Text style={NH.title}>NEXUS <Text style={{ color:CYAN }}>HQ</Text></Text>
          </View>
...
const NH = StyleSheet.create({
  root: { backgroundColor:'#060B12', overflow:'hidden' },
...
// Telemetry card
const TCard = memo(({ title, icon, color, big, sub, children }: {
  title:string; icon:string; color:string; big:string; sub:string; children?:React.ReactNode;
}) => (
  <View style={[TC.card, { borderTopColor:color, borderColor:color+'30' }]}>
    ...
    <View style={{ flex:1, backgroundColor:BG }}>
      ...
      >
        ...
        <View style={{ flexDirection:'row', flexWrap:'wrap', gap:8 }}>
          <TCard title="CONNECTED PC" icon="desktop-classic" color={cc}
          ...
          <TCard title="LIVE FEED" icon="pulse" color={PURP}
          ...
          <TCard title="CRAWLER GRAPH" icon="chart-timeline-variant" color={CYAN}
          ...
          <TCard title="KNOWLEDGE" icon="brain" color={GREEN}
        ...
        <View>
          <View style={{ flexDirection:'row', alignItems:'center', gap:6, marginBottom:8 }}>
            ...
            <Text style={{ fontFamily:MONO, fontSize:9, color: CYAN+'80', fontWeight:'900', letterSpacing:1.8 }}>CORE SURFACES</Text>
        ...
        {/* Status footer */}
        <View style={[FK.root, { borderColor: cc+'30' }]}>
