...
function KBInner() {
  ...
  useFocusEffect(useCallback(() => {
    ...
    knowledgeAccumulator.getStats?.()
      .then(s => { if (s?.totalFindings) setTotal(s.totalFindings + ALL_FACTS.length); })
      .catch(() => {});
  }, []));
  ...
  const filtered = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      list = list.filter(f => f.text.toLowerCase().includes(q) || f.tags.some(t => t.includes(q)));
    }
    return list;
  }, [catFilter, query, showStarred, starred]);
