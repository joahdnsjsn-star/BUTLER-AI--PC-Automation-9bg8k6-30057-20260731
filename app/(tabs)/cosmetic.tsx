...
function CosmeticScreen() {
  ...
  return (
    <View style={[s.root, { paddingTop: insets.top }]}>
      <View style={s.header}>
        <MaterialCommunityIcons name="palette-swatch" size={22} color={D.pink} />
        <Text style={[s.title, { color: D.pink }]}>SKINS</Text>
        <Text style={s.sub}>Theme & Visual Customisation</Text>
      </View>
      <View style={s.body}>
        <MaterialCommunityIcons name="palette-swatch" size={48} color={D.textDim} />
        <Text style={s.comingSoon}>COMING SOON</Text>
        <Text style={s.hint}>Custom colour schemes, fonts and UI themes</Text>
      </View>
    </View>
  );
}
...
export default function Cosmetic() {
  return (
    <TabErrorBoundary tabName="SKINS">
      <CosmeticScreen />
    </TabErrorBoundary>
  );
}
