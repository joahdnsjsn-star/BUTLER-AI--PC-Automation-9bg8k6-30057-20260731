...
function ConnectInner() {
  ...
  const connect = async () => {
    ...
    try {
      const r: any = serverConnection.connectManual
        ? await (serverConnection as any).connectManual(ip.trim(), '8766')
        : { success: false, error: 'N/A' };
  ...
  const disconnect = () => {
    ...
    setIsConn(false); setStatus('Disconnected');
  };
...
const CH = StyleSheet.create({
  root:      { backgroundColor:'#05090F', overflow:'hidden' },
