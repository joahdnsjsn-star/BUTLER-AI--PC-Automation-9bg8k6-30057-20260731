    ...
    <View style={[FH.root, { paddingTop:safeTop }]}>
      ...
      <View style={FH.body}>
        <View style={{ flex:1, gap:4 }}>
          <Text style={FH.eye}>PYTHON AUTOMATION · {total} SCRIPTS</Text>
          <View style={{ flexDirection:'row', alignItems:'center', gap:8 }}>
            <MaterialCommunityIcons name="code-braces-box" size={18} color={BLUE} />
            <Text style={FH.title}>SCRIPT <Text style={{ color:BLUE }}>FORGE</Text></Text>
          </View>
          ...
          <View style={[RM.outputBox, { borderColor:color+'30', backgroundColor:color+'05' }]}>
            {running
              ...
              : <ScrollView showsVerticalScrollIndicator={false}>
                  <Text style={[RM.output, { color: ok ? '#88FFBB' : '#FF9090' }]} selectable>{result || 'No output'}</Text>
                </ScrollView>
            }
          </View>
  ...
  const runScript = useCallback(async (s: Script) => {
    if (s.danger) {
      ...
      await new Promise<void>(res => {
        Alert.alert(
          '⚠ CAUTION',
          `"${s.name}" can modify system settings. Continue?`,
          [
            { text: 'Cancel', style: 'cancel', onPress: () => res() },
            { text: 'RUN', style: 'destructive', onPress: () => res() },
          ]
        );
      });
    }
...
export default function ScriptsScreen() {
  return <TabErrorBoundary name="Scripts"><ScriptsInner /></TabErrorBoundary>;
}
